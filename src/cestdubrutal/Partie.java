...
public class Partie {
	...
	public static void main(String[] args) {
		...
		//------------
		//ETAPE 2
		ZoneInfluence[] tabZI = new ZoneInfluence[5];
		tabZI[0] = new ZoneInfluence(ZoneInflu.BU);
		tabZI[1] = new ZoneInfluence(ZoneInflu.BDE);
		tabZI[2] = new ZoneInfluence(ZoneInflu.ADMIN);
		tabZI[3] = new ZoneInfluence(ZoneInflu.HALLESINDUS);
		tabZI[4] = new ZoneInfluence(ZoneInflu.HALLESPORTIVE);
		...
		for(int i=0;i<2;i++) {
			for(ZoneInfluence ZI:tabZI) {
				System.out.println("Combien d'élève sur :"+ ZI.getNom().toString());
...
//		tabZI[1].ajouterEleve(j2.getTabEleve()[1]);
		...
		for(int indiceZoneInflu=0;indiceZoneInflu<5;indiceZoneInflu++) {
			...
			tabZI[indiceZoneInflu].rangerListeMeilleurInitiative();
			
			//pour chaque élève on va attaquer ou soigner les autres
			for(int indiceEleve=0;indiceEleve<tabZI[indiceZoneInflu].indiceTabEleve;indiceEleve++) {
				System.out.println(tabZI[indiceZoneInflu].getTabEleveZone()[indiceEleve].getStrat());
				
				//pour les joueurs avec comme Strat attaquer
				if(tabZI[indiceZoneInflu].getTabEleveZone()[indiceEleve].getStrat()==Eleve.Strategie.Attaquer) {
					...
					for(int j=0;j<tabZI[indiceZoneInflu].indiceTabEleve;j++) {
						
						//si l'élève est mort continue
						if(tabZI[indiceZoneInflu].getTabEleveZone()[j].isVire()) {
							continue;
						}
						
						//si l'élève n'appartient pas au même joueur 
						if(!(tabZI[indiceZoneInflu].indiceEleveMemeJoueur(indiceEleve, j, j1, j2))) {
						...
						if(indiceCombat==1) {
							tabZI[indiceZoneInflu].getTabEleveZone()[indiceJoueurAdverse[0]].setECTS(tabZI[indiceZoneInflu].getTabEleveZone()[indiceJoueurAdverse[0]].getECTS()-10);
							//verif si le reuf est plus à l'UTT
							tabZI[indiceZoneInflu].getTabEleveZone()[indiceJoueurAdverse[0]].estilvire();
						}
							...
							for(int indiceComb=1;indiceComb<indiceCombat;indiceComb++) {
								if(tabZI[indiceZoneInflu].getTabEleveZone()[indiceJoueurAdverse[indiceMinEcts]].getECTS()>tabZI[indiceZoneInflu].getTabEleveZone()[indiceJoueurAdverse[indiceComb]].getECTS()) {
							...
							tabZI[indiceZoneInflu].getTabEleveZone()[indiceMinEcts].setECTS(tabZI[indiceZoneInflu].getTabEleveZone()[indiceMinEcts].getECTS()-10);
							//verif si le reuf est plus à l'UTT
							tabZI[indiceZoneInflu].getTabEleveZone()[indiceMinEcts].estilvire();
